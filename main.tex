...
\renewcommand\thepart{\Alph{part}} % Inlagd av Erik 
...
\newcommand{\titel}{Små matematiska såll och dess tillämpningar} %Skriv in projektets/rapportens titel här
\newcommand{\undertitel}{En introduktion och algoritmisk implementation} %Skriv in ev undertitel här, eller lämna tomt
\newcommand{\engtitel}{Small Mathematical Sieves and their Applications} %Skriv in engelsk översättning av projektets/rapportens titel här
...
\section{Ordonotation} 
    \input{erik/ordo} \label{APDX:Ordo}

\section{Bevis av några resultat som används}
    \subsection{Partiell summation}
    \subsection{Bevis av (\ref{Thm.1.4.4}) och 
                \texorpdfstring{\(\sum_{p \leq z} \frac{1}{p} = \log \log z + O(1)\)}{Summan av primtalsreciproker}}
    \input{erik/Thm.1.4.3-4}
    
    \subsection{Ett lemma angående multiplikativa funktioner}
    \input{coen/lemmaMult}
    
    \subsection{En variant på Möbius inverteringsformel}
    \input{coen/mobDual}
    
\section{Kedjebråk}
    \input{erik/kedjebrak} \label{APDX:cfrac}
