...
Legendres formel är bra för att räkna primtal exakt men är långsam och golvfunktionen kan vara svårhanterlig. Vi kan underlätta för oss genom att ersätta golfunktionen med $x/d$ och dess bråkdel, \(\{x/d\} = O(1)\), som restterm vilket leder till \(\A_d = x/d + O(1)\). Dessvärre resulterar det här i en väldigt stor felterm då restermerna ackumulerar, så att \(\pi(x) - \pi(\sqrt{x}) + 1 = x \sum_{d \divides P(z)}\frac{\mu(d)}{d} + O(2^{\pi(\sqrt{x})}) \).
