...
\end{align*}
där sista steget följer av olikheten \(\log(1 + x) \leq x, \forall x > -1\). Slutligen ger en partiell summation av (\ref{Thm.1.4.4}) \todo{ev. bevisa i appendix} oss att huvudtermen är \(\ll (\log z)^{-2}\) och väljer vi \(\log z = \log x / A \log \log x\) så erhåller vi \cite[Sats 5.4.4]{cojocarumurty},
