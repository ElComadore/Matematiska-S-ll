...
De två summorna innanför parentesen är vår felterm, vi fokuserar på den andra summan. Vi kan skriva om den, först med hjälp av triangelolikheten och sedan med $\abs{\mu(x)} \leq 1$ och vårt antagande $\abs{R_d} = O(\omega(d))$ så att
\begin{align*}
...
\end{align*}
vari sista steget använder vi \(\log(1 + x) \leq x\). Väljer vi \(\delta = 1 - 1/\log z\) och utnyttjar olikheten \(e^x \leq 1 + x e^x\) för $x \geq 0$, och sist partiell summering av (\ref{dimension}) får vi att
\begin{align*}
