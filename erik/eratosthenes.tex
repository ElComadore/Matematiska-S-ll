...
\begin{align*}
    \S{A}{P}{z} = \left(X W(z) \right) + \left(- \sum_{\substack{d \divides P(z) \\ d > y}} \frac{\omega(d)}{d} \mu(d) + \sum_{\substack{d \divides P(z)  \\ d \leq y}} \mu(d) R_d \right).
\end{align*}
...
Sammanställt, huvud- och felterm, få vi följande sats:

\begin{theorem}[Eratosthenes generaliserade såll]\label{thm:EratosthenesSieve}
Med notationen från avsnitt \ref{sallproblemet} och antagandena nämnda tidigare i detta avsnitt, då gäller att
\begin{align*}
