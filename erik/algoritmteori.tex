...
Vi kan ställa ytterligare krav på \(r \in [-R, R]\) för att slippa gå över hela intervallet. Vad \cite{HaraldSieve} gör är att, med en diofantisk approximation \todo{förklara}, hitta \(a, q\) med \(\gcd{a,q} = 1\) som approximerar \(\alpha_1 := - n / m_0^2\) så att \(\abs{\alpha_1 - \frac{a}{q}} \leq \frac{1}{q \cdot 2R}\) för största möjliga konvergent $q$ mindre än eller lika med \(2R\). Låt \(\alpha_0 = n / m_0\), då får vi att
\begin{align*}
