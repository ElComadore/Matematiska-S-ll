...
\begin{theorem}
    Låt \((a_n)_{n=0}^{N}\) vara en följd av heltal med positiva termer då \(n > 0\) och med konvergenter \(p_n / q_n = c_n\). Kedjebråket \(\xi = \langle a_0, a_1, \dots, a_N \rangle\) uppfyller då för alla \(0 \leq n < N/2\)
    \begin{enumerate}
        \item \(c_{2n} < \xi < c_{2n + 1}\) samt
        \item \(\frac{a_{n + 2}}{q_n q_{n + 2}} < \abs{\xi - c_n} < \frac{1}{q_n q_{n + 1}}\) % Undre gräns överflödig?
    \end{enumerate}
\end{theorem}
\begin{proof}
