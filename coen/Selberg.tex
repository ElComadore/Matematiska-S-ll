...
Enligt \cite{cojocarumurty}, bestod grundläggande uppskattningen av Selbergs såll huvudsakligen av att för en given reell talföljd, \((\lambda_d)\), med \(\lambda_1 = 1\), har vi att 
\begin{equation}
...
För att få en form på \eqref{sel.sieve.main} vilken vi kan använda för att uppskatt mängders storlek kommer vi nu bestämma talföljden \((\lambda_d)\). 
...
%Vid detta minimumvärde blir \(|\lambda_d|\leq 1\) vilket då också ger oss en uppskattning av feltermen. Med alla dessa omskrivningar härleder vi följande sats.
 Med det som utgångspunkt, börjar vi med att använda oss av ett lemma som låter oss skriva om multiplikativa funktioner av minsta gemensamma multipler, se Appendix \ref{APDX:multFunk}.
