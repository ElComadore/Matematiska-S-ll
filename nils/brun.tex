...
Genom en variant av Möbius inverteringsformel gäller det att
\begin{equation*}
    \sum_{d\mid n} \mu\left(\frac{n}{d}\right) f(d) = \mu(n)g(n).
\end{equation*}
Viggo Brun använde detta för att modifiera (\ref{inclusionexclusion}) och visade att för varje funktion $g$ som uppfyller $g(1)=1$, vilket implicerar $f(1)=1$, så gäller det att
\begin{equation*}
    \sum_{d \mid P(z)} \mu(d) g(d) \card\A_d = \S{A}{P}{z} - G,
\end{equation*} % G = \sum_{d \mid P(z)} \sum_{\substack{p \mid P(z) \\ p<q(d)}} \mu(d) (g(d)-g(pd)) S(\A_{pd},\P^{(pd)},p).
där $G$ är en funktion av $\A,\P$ och $z$, definierad med hjälp av $g$. 
