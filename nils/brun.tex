...
Därefter konstruerade han två funktioner $g_U$ och $g_L$ så att $\mu(d)(g_U(d)-g_U(pd))\geq0$ och $\mu(d)(g_L(d)-g_L(pd))\leq0$ håller för alla $d \mid P(z)$ och $p<q(d)$.
...
\begin{comment}
Vi nöjer oss med att beskriva huvudidéen bakom sållet med stöd i \cite[kap 6.2]{cojocarumurty}. %då en noggrann härledning inte skulle få plats i denna rapport.
...
\begin{equation}\label{brun.eq.gugl}
    ...
    \leq \sum_{d \mid P(z)} \mu(d) g_U(d) \card\A_d.
\end{equation}
Enligt \cite{cojocarumurty} var detta Bruns innovativa idé. %Valet för $g_U$ och $g_L$ som antar värdena 1 eller 0 beronde på delarna i $d$.
...
Precis som i avsnitt \ref{eratosthenes.tillämpning} vill vi sålla bort tal i kongruensklasserna $0$ och $-2$ modulo $p$, 
...
För att visa att $\kappa=2$ uppfyller det sista antagandet i \ref{brun.thm.brun} använder vi oss av (\ref{Thm.1.4.4}) som tillsammans med $\omega(p)\leq2$ implicerar
\begin{equation*}
