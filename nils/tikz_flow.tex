...
\def\H{0.7cm}     % Min height of objects
\def\W{1.7cm}     % Min width of objects
\def\distV{1.1cm} % Vertical distance between objects
\def\distH{0.7cm} % Horizontal distance between objects
...
% Objects
\tikzstyle{startstop} = [rectangle, minimum width=\W, minimum height=\H, text centered, draw=black, rounded corners=\H/2]
...
% Objects
    ...
    \node (sim1)  [process,  right=\distH of sim2]  {\textsc{SimpleSiev}};
    \node (sim0)  [process,  below=-1pt   of sim1]  {\textsc{SimpleSegSiev}};
    \node (A)     [decision, below=\distV of sim1]  {\textbf{A}};
    % Middle
    \node (B0)    [decision, right=\distH of A]     {\textbf{B}};
    % Second loop
