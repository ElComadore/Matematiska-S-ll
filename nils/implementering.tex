...
Som nämnts tidigare så sållar programmet fram primtal i ett angivet intervall $[n-\Delta,n+\Delta]$. 
Metoden som programmet gör detta med varierar något beroende på förhållandet mellan $\Delta$ och $n$,
av denna anledning utfördes två stycken tester där detta förhållande är så stort som möjligt respektive så litet som möjligt.
...
För alla mätningar visade sig den förbättrade versionen vara minst 10 gånger så snabb som den första versionen, det finns dessutom en antydan till att denna faktor ökar då $n$ växer. De uppmätta tiderna går att se i tabell \ref{implementering.tidtabell}.

\begin{table}[H]
\centering
\input{nils/tidtabell}
\caption{
Uppmätta körningstider vid primtalssållning för den första versionen, respektive den förbättrade versionen av programmet.
Intervallen är på formen $[n-\Delta,n+\Delta]$ där $\Delta=n$ för de tre första och $\Delta=\sqrt[3]{n}$ för de tre sista.
Den förbättrade versionen var snabbare än den första versionen med en faktor på minst 10.}
